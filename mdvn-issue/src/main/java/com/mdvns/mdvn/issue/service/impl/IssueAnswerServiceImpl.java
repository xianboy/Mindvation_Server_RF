...
import com.mdvns.mdvn.issue.service.IssueAnswerService;
 ...
 */
@Service
public class IssueAnswerServiceImpl implements IssueAnswerService {

    /* 日志常亮 */
    private static final Logger LOG = LoggerFactory.getLogger(IssueAnswerServiceImpl.class);
    ...
    public RestResponse<?> createIssueInfo(CreateIssueRequest request) throws BusinessException {
        ...
        issue.setProjSerialNo(request.getProjId());
        issue.setIsResolved(0);
        issue = this.issueRepository.saveAndFlush(issue);
        issue.setIssueId("IS" + issue.getId());
        issue = this.issueRepository.saveAndFlush(issue);
        ...
        rtrvRequest.setProjId(issue.getProjSerialNo());
...
//        return rtrvIssueDetail(rtrvRequest);
    ...
    public RestResponse<?> createIssueAnswerInfo(CreateIssueAnswerRequest request) throws BusinessException {
        ...
        issueAnswer.setProjSerialNo(request.getProjId());
        issueAnswer = this.issueAnswerRepository.saveAndFlush(issueAnswer);
        issueAnswer.setAnswerId("AS" + issueAnswer.getId());
        issueAnswer = this.issueAnswerRepository.saveAndFlush(issueAnswer);
...
//            issueDetail.setIssueInfo(null);
        ...
        if (null == issue) {
            ...
            for (int i = 0; i < issueAnswers.size(); i++) {
                ...
                //查询创建者对象信息
                Staff answerCreatorInfo = this.rtrvStaffInfoById(issueAnswer.getCreatorId());
